...
export class ShowdownPreviewer {
        ...
        (message) => {
          switch (message.command) {
            ...
            case 'resetImagePath':
              ...
              break;
            case 'getFileContent':
              this.getFileContent(message.args[0]);
              break;
          }
        },
        null,
  ...
  public async getFileContent(data: FileSource) {
    const that = this;
    if (data.src) {
      const srcUri = vscode.Uri.parse(data.src);
      const content = await utils.readFile(srcUri.fsPath, { encoding: 'utf-8' });
      that.webpanel?.webview.postMessage({ command: 'onGetFileContent', id: data.id, response: content });
    }
  }
  ...
  public async saveLocalHtml(
    ...
    // const showdownsScript = await utils.readFile(
    //   path.join(this.context.extensionPath, 'node_modules/@jhuix/showdowns/dist/showdowns.min.js'),
    //   {
    //     encoding: 'utf-8'
    //   }
    // );
    ...
    let outerScripts = '';
    let innerScripts = '';
    let bodyScripts = ''; // `<script id='script-showdowns'>${showdownsScript}</script>\n`;
  ...
  if (!('${o.name}' in window)) {
    ...
    if (this.config.showPageLabel) {
      ...
      bottomComment = `<div class="extension" id="extension_info"><div>
      ...
      if (!slient) {
        ...
        vscode.window
          .showInformationMessage(
            localize(this.config.locale, 'msg.createdfile', path.basename(dest), dest),
            actionItem
          )
          .then((act) => {
            if (act === actionItem) {
...
<style id="_baseStyles" type="text/css">
  * {
  ...
  .vscode-workspace-container .showdowns code {
    background-color: var(--vscode-textPreformat-background);
  }
  .vscode-workspace-container .showdowns pre code {
