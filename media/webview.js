  ...
  (function(previewer, options) {
    ...
    class PreviewHtml {
      ...
      updateOptions(options2) {
        ...
        if (options2.tex) {
          previewer.setExtensionOptions("tex", options2.tex);
        }
        if (options2.gnuplot) {
          previewer.setExtensionOptions("gnuplot", options2.gnuplot);
        }
      }
      getOtherStyles() {
        let styles = [];
      ...
      renderPlantuml(id, code, options2) {
        const that = this;
        return new Promise((resolve, reject) => {
          that.resolveCallbacks[id] = { resolve, reject };
          that.postMessage("renderPlantuml", [{ id, code, options: options2, sourceUri: that.sourceUri }]);
        });
      }
      renderKroki(id, code, options2) {
        const that = this;
        return new Promise((resolve, reject) => {
          that.resolveCallbacks[id] = { resolve, reject };
          that.postMessage("renderKroki", [{ id, code, options: options2, sourceUri: that.sourceUri }]);
        });
      }
      renderTex(id, code, options2) {
        const that = this;
        return new Promise((resolve, reject) => {
          that.resolveCallbacks[id] = { resolve, reject };
          that.postMessage("renderTex", [{ id, code, options: options2, sourceUri: that.sourceUri }]);
        });
      }
